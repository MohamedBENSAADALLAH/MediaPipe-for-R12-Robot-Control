...
#RBinitCMD=['connect','ROBOFORTH','START','READY', 'HOME' ]
#RBCMD=['READY', 'HOME' ]
...
class ArmShell(cmd.Cmd, object):
    ...
    def cmdloop(self, RBCMD,intro=None):                         # modified version by ghoggali to make r12 run in automatique way     
        ''' Override the command loop to handle Ctrl-C. '''
       
        self.preloop()
        #print('cmdlop')
    
        ''' TO CHECK THE STATE OF THE ROBOT ADDED BY US!
        for x in RBCMD:
            print(x, '   ')
        '''    
        for x in RBCMD:
          line = x
 	  #print('\n line147---------------------\n')   ##########################################
          line = self.precmd(line)    #this is to update the prompt after each iteration
          #print('\n-----------------------------------\n')
          #print('\nline150',line)
          #print('\n-----------------------------------\n')
          #print('\n-----------------------------------\n')
    ...
    def cmdloop_oldversion(self, intro=None):
        ''' Override the command loop to handle Ctrl-C. '''
        self.preloop()
        #print('cmdlop')
